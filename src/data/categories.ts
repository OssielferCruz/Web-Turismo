import { CategoryDetails } from "../types/site";

export const CATEGORIES: Record<string, CategoryDetails> = {
  "Patrimonio UNESCO":  { color: "#c2622a", accent: "#fff4ee", label: "Patrimonio UNESCO" },
  "Volcán":             { color: "#4b5563", accent: "#f3f4f6", label: "Volcán / Naturaleza" },
  "Arqueología":        { color: "#7c5c38", accent: "#fdf8f2", label: "Arqueología" },
  "Museo":              { color: "#1d4ed8", accent: "#eff6ff", label: "Museo" },
  "Playa":              { color: "#0e7490", accent: "#ecfeff", label: "Playa / Ecoturismo" },
  "Iglesia":            { color: "#b45309", accent: "#fffbeb", label: "Iglesia Colonial" },
  "Teatro":             { color: "#7c3aed", accent: "#f5f3ff", label: "Teatro" },
  "Salud / Historia":   { color: "#be185d", accent: "#fdf2f8", label: "Salud / Historia" },
  "Universidad":        { color: "#15803d", accent: "#f0fdf4", label: "Universidad" },
  "Parque":             { color: "#16a34a", accent: "#f0fdf4", label: "Parque / Plaza" },
};

export function getCategoryColor(category: string): string {
  return CATEGORIES[category]?.color ?? "#6b7280";
}

export function getCategoryAccent(category: string): string {
  return CATEGORIES[category]?.accent ?? "#f9fafb";
}
